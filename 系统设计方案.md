...
系统面向"家庭"这一记账主体，提供家庭收入/支出的**录入、编辑、查询、统计、分析**全流程管理，并拓展**家庭资产管理**（房产、存款、汽车、房/车贷款等）能力，帮助家庭了解收支状况、发现消费问题、优化财务决策。

### 1.2 建设目标

- **基本目标**：完成家庭收支的 CRUD（增删改查）+ 多维统计 + 基础分析；
- **进阶目标**：区分"统计"与"分析"，输出分析报告与异常预警；支持自定义多级收支分类；
- **拓展目标**：家庭资产管理（房产 / 存款 / 汽车 / 贷款）；
...
| F9 | 资产管理（拓展） | 房产、存款、汽车等资产登记与总资产统计 | P1 |
...
|------|------|------|
| JDK | 17（LTS） | 长期支持版 |
| 框架 | Spring Boot 4.1.1 | 简化配置、内嵌 Tomcat、生态成熟（官方 Initializr 现默认版本，3.x 已过支持期下架；MyBatis-Plus/Sa-Token 均已发布 Boot 4 官方适配版） |
...
erDiagram
    ...
    asset {
        bigint id PK
        bigint family_id FK
        varchar asset_type "房产/存款/汽车/其他"
        decimal value "价值"
    }
    loan {
        bigint id PK
...
| asset_type | VARCHAR(20) | NOT NULL | 房产 / 存款 / 汽车 / 其他 |
...
Java 17 + Spring Boot 4.1.1 + MyBatis-Plus 3.5.17（spring-boot4-starter）+ MySQL 8（5.7 兼容）+ Sa-Token 1.46.0（spring-boot4-starter）+ knife4j + Lombok。
...
| 资产管理 | 资产汇总卡（总资产/按类型分布）+ 资产表格 + 贷款表格 | 贷款还款计划测算 |
...
- "礼金红包"等收入分类的流水可**关联**资产对象（`transaction.asset_id` 可空字段，v1 暂未实现）；
- 首页净资产卡片 = 总资产 − 总贷款。
...
- 部分项目含**股票/基金行情与持仓盈亏**模块，需每日维护行情数据并开发专用行情接口，维护成本高、演示繁琐；本项目调研后**不纳入**该类资产，资产模块聚焦房产/存款/汽车等静态登记，搭配贷款、预算两项拓展。
